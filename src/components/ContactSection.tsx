...
const ContactSection = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const text = `Nova Mensagem de Contato (Website)\n\nNome: ${form.name}\nEmpresa: ${form.company || 'Não informada'}\nEmail: ${form.email}\nTelefone: ${form.phone || 'Não informado'}\n\nMensagem:\n${form.message}`;
    const encodedText = encodeURIComponent(text);
    const whatsappUrl = `https://wa.me/258856494575?text=${encodedText}`;
    ...
    <section id="contato" className="section-padding bg-muted/20">
      <div className="container mx-auto">
        ...
        >
          ...
          <h2 className="font-display text-4xl md:text-5xl font-bold mt-4 mb-8 text-foreground">{t('contact.title')}</h2>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-5 gap-12">
          ...
          >
            <div className="grid sm:grid-cols-2 gap-6">
              <input required placeholder={t('contact.form.name')} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-white border border-border rounded-xl px-6 py-4 text-sm font-bold text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-primary transition-all focus:ring-4 focus:ring-primary/10 shadow-md" />
              <input placeholder={t('contact.form.company')} value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="w-full bg-white border border-border rounded-xl px-6 py-4 text-sm font-bold text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-primary transition-all focus:ring-4 focus:ring-primary/10 shadow-md" />
            </div>
            <div className="grid sm:grid-cols-2 gap-6">
              <input required type="email" placeholder={t('contact.form.email')} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full bg-white border border-border rounded-xl px-6 py-4 text-sm font-bold text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-primary transition-all focus:ring-4 focus:ring-primary/10 shadow-md" />
              <input placeholder={t('contact.form.phone')} value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full bg-white border border-border rounded-xl px-6 py-4 text-sm font-bold text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-primary transition-all focus:ring-4 focus:ring-primary/10 shadow-md" />
            </div>
            <textarea required rows={5} placeholder={t('contact.form.message')} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="w-full bg-white border border-border rounded-xl px-6 py-4 text-sm font-bold text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-primary transition-all focus:ring-4 focus:ring-primary/10 resize-none shadow-md" />
            <motion.button 
              type="submit" 
              disabled={sending} 
            ...
            >
              <Send size={24} /> {sending ? t('contact.form.sending') : t('contact.form.send')}
            </motion.button>
          </motion.form>
          ...
          >
            <div className="bg-white border border-border rounded-3xl p-6 md:p-10 shadow-lg overflow-hidden">
              <h3 className="font-display text-xl md:text-2xl font-bold mb-6 md:mb-8 text-foreground">{t('contact.info.title')}</h3>
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 text-muted-foreground group">
                  ...
                  <span className="font-bold text-foreground text-sm md:text-base">{t('contact.info.address')}</span>
                </div>
              </div>
            </div>
            <motion.a 
